...
import {NewOperator as NewOperatorAPI} from "../../../controller/OperatorController";
...
class NewOperator extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            className_email: 'form-control form-control-sm',
            className_pass: 'form-control form-control-sm',
    ...
    handleSubmit = async (event) =>{
        ...
        if(new RegExp(/[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,15}/g).test((this.state.email))) {
            ...
            this.setState({className_email: "form-control form-control-sm is-valid"});
            mail_valido = true
        } else {
            this.setState({msj_error_email: 'Ingrese un e-mail válido.'});
            this.setState({className_email: "form-control form-control-sm is-invalid"});
            mail_valido = false
        }

        if(this.state.password === this.state.confirm_password) {
            this.setState({msj_error_pass: ''});
            this.setState({className_pass: "form-control form-control-sm is-valid"});
            passwords_validas = true
        } else {
            this.setState({msj_error_pass: 'Las contraseñas ingresadas no son iguales.'});
            this.setState({className_pass: "form-control form-control-sm is-invalid"});
        ...
        if(mail_valido && passwords_validas) {
            ...
            let newOperator = await NewOperatorAPI(name, lastName, pass, email, rol, companyId);

            if(newOperator.rdo === 201 ) {
                                        ...
                                        <div className="col-sm-12 col-md-10 col-lg-10 align-self-center"> 
                                            <div className="card mt-5">
                                                <div className="card-body text-center mt-4 mb-4">
                                                    ...
                                                    <h2>No pudimos crear el usuario</h2>
